...
Widget::Widget(QMenuBar *parent):QMenuBar(parent)

 {
    ...
    coss=new QPushButton("COS");
    sins=new QPushButton("SIN");
    drob=new QPushButton("1/x");
    sqrt=new QPushButton("√");
    //proc=new QPushButton("%");
    ...
    connect(coss,SIGNAL(clicked()),this,SLOT(cossb()));
    connect(sins,SIGNAL(clicked()),this,SLOT(sinsb()));
    connect(drob,SIGNAL(clicked()),this,SLOT(drobb()));
    connect(sqrt,SIGNAL(clicked()),this,SLOT(sqrtb()));
    //connect(proc,SIGNAL(clicked()),this,SLOT(procb()));
    this->setLayout(vbox);
...
void Widget::cossb(){
    QString s=line->text();
    fnum=s.toDouble();
    double c=cos(fnum*M_PI/180);
    line->setText(QString::number(c));
}
void Widget::sinsb(){
    QString s=line->text();
    fnum=s.toDouble();
    double c=sin(fnum*M_PI/180);
    line->setText(QString::number(c));
}
void Widget::drobb(){
    QString s=line->text();
